...
//            size = sizeof(volume);
        ...
        var muteValue:UInt32 = volume == 0.0 ? 1 : 0
        address.mElement = channels[ 0 ];
        err = AudioObjectSetPropertyData( deviceID, &address, 0, nil, size, &muteValue );
        ...
        err = AudioObjectGetPropertyData( deviceID, &address, 0, nil, &size, &name );
    ...
    class func getDefaultInputDeviceName()-> String{
        var address:AudioObjectPropertyAddress
        var deviceID: AudioDeviceID = kAudioDeviceUnknown
        var err: OSStatus
        var size: UInt32 = 0
        ...
        err = AudioObjectGetPropertyData( deviceID, &address, 0, nil, &size, &result );
...
// Addditional helper methods  https://gist.github.com/rlxone/584467a63ac0ddf4d62fe1a983b42d0e
